...
const API_BASE = "/api/pp/field-files";
...
async function uploadOneFieldMultipart(
  entry: BatchEntry,
  regionId: string,
  employeeId: string,
  def: string | undefined,
...
export async function ppFieldUploadFilesBatch(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (fileRequiresMultipartUpload(it.file.size)) {
      await flushBatch();
      const mr = await uploadOneFieldMultipart(entry, regionId, employeeId, def, uploadDate, cb);
