...
/**
 * DT and Driver/Rigger slots on any team (labels include team name).
 * Used when an admin-portal user assigns from the employee portal without regional limits.
 */
export async function loadAllTeamAssigneeOptions(supabase: SupabaseClient): Promise<{ id: string; label: string }[]> {
  const teams = await fetchAllTeamsWithSlots(supabase);
  if (teams.length === 0) return [];
...
/** True if the employee is DT or Driver/Rigger on any team. */
export async function targetEmployeeIsOnAnyTeam(supabase: SupabaseClient, targetEmployeeId: string): Promise<boolean> {
  const teams = await fetchAllTeamsWithSlots(supabase);
  return teams.some(
    (t) => t.dt_employee_id === targetEmployeeId || t.driver_rigger_employee_id === targetEmployeeId
  );
}

/**
 * Active employees company-wide (primary region label), optional QC exclusion — portal admin region mode.
 */
export async function loadAllRegionEmployeeAssigneeOptions(
  supabase: SupabaseClient,
  options: PmRegionAssigneeOptions
): Promise<{ id: string; label: string }[]> {
  const { data: emps, error } = await supabase
...
export async function targetEmployeeIsGlobalRegionAssignee(
  supabase: SupabaseClient,
  targetEmployeeId: string,
  validation: PmRegionTargetValidation
): Promise<boolean> {
