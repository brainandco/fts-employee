...
import { loadPmWhoHasAssetsData } from "@/lib/mobile/pm-assign-data";
import { requirePmMobileContext } from "@/lib/mobile/require-pm-mobile";
import { getRequestAuth } from "@/lib/supabase/request-auth";

/** GET — PM who has assets in region scope + receipt status (Bearer). */
export async function GET(req: Request) {
  ...
  const data = await loadPmWhoHasAssetsData(
    supabase,
