...
import { getS3ForPpReportsZip, resolveMultiPpReporterFolders } from "@/lib/pp-reports/folder-zip";
...
/** POST { paths: string[] } — zip multiple folders under the reporter’s tree. */
export async function POST(req: Request) {
  ...
  const resolved = resolveMultiPpReporterFolders(gate.reporterFolderSlug, paths);
  if (!resolved.ok) {
