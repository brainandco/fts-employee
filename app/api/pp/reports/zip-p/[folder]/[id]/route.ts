...
/** Short public ZIP: `/api/pp/reports/zip-p/{folderName}/{linkId}` */
export async function GET(_req: Request, ctx: { params: Promise<{ folder: string; id: string }> }) {
  ...
  const { row } = fetched;
  if (row.link_kind !== "reporter" || !row.reporter_slug) {
  ...
  const resolved = resolvePpReporterFolderZip(row.reporter_slug, row.normalized_folder_path);
  if (!resolved.ok) {
